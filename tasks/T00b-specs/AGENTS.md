...
- Без runtime-кода.
- без parser/playback реализации.
- без transport integration.
...
- единый стиль полей и инвариантов
- versioning notes и cross-links
...
```bash
test -d specs/objects
grep -R '^# ' specs/objects | head
grep -R '## Инварианты' specs/objects | head
```
