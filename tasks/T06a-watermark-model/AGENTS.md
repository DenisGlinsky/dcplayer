# AGENTS.md — T06a Watermark Model
...
- Без actual detector harness.
- без GPU insertion.
- без audio FM implementation.
...
- формальная watermark model
- attack matrix baseline
- control flags/state machine
...
```bash
./scripts/build.sh
./scripts/test.sh -R 'watermark_model|fm_state'
```
