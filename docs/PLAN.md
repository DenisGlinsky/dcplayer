...
Дополнительное правило именования: task directory и task label должны иметь формат `TXXx-слово` или `TXXx-слово-слово`. Длинные многочастные имена запрещены.
...
**Цель**  
Собрать CPU Decoder как baseline/oracle и fallback.
...
**Цель**  
Реализовать SMPTE Text path, включая fonts и timing.
...
**Definition of Done**  
SMPTE Text корректно обрабатывается вместе со шрифтами и временными событиями.
